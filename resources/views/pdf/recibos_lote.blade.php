...
<head>
    <meta charset="utf-8">
    <title>Compendio de Recibos - {{ $periodo_nombre ?? 'Período' }}</title>
    <style>
        @page {
        ...
        /* Ficha del Propietario e Inmueble */
        .info-grid-table {
            width: 100%;
            border-collapse: collapse;
            margin-bottom: 6px;
        ...
        /* Resumen Financiero Side-by-Side */
...
<body>
    ...
    @foreach($items as $idxItem => $reciboData)
        @php
            $invoice = $reciboData['invoice'];
            $condominio = $reciboData['condominio'];
            $apartamento = $reciboData['apartamento'];
            $propietario = $reciboData['propietario'];
            $alicuota_percent = $reciboData['alicuota_percent'];
            $doc_numero = $reciboData['doc_numero'];
            $periodo_nombre = $reciboData['periodo_nombre'];
            $fecha_emision = $reciboData['fecha_emision'];
            $gastos = $reciboData['gastos'];
            $es_borrador = $reciboData['es_borrador'];
            $es_extraordinario = ($invoice->tipo_recibo === 'extraordinario');
            $cuentasBancarias = $reciboData['cuentasBancarias'] ?? [];
        @endphp
                ...
                <td style="width: 38%; vertical-align: middle;">
                    <div class="doc-badge-box" style="{{ $es_extraordinario ? 'border-color: #c4b5fd; background-color: #fbfbfe;' : '' }}">
                        ...
                        <div class="doc-meta-sub">
                            N° DOC: <strong>{{ $doc_numero }}</strong>
                            <br>
                            Emisión: {{ $fecha_emision }} | Vence: 5 días
                        </div>
                    </div>
                </td>
            </tr>
        ...
        <table class="info-grid-table">
            <tr>
                ...
                <td class="info-cell" style="width: 44%;">
                    ...
                    <div style="font-size: 7px; color: #64748b; margin-top: 1px;">
                        C.I. / RIF: <strong>{{ $propietario->cedula ?? 'V-00000000' }}</strong> | {{ $propietario->email ?? 'contacto@condominio.com' }}
                    </div>
                </td>
                @php
        ...
        <div class="table-section-title">{{ $es_extraordinario ? 'Desglose del Presupuesto Extraordinario' : 'Estructura de Gastos y Conceptos del Período' }}</div>
            ...
            <tbody>
                ...
                @forelse($gastosComunes as $idx => $gasto)
                    ...
                    <tr class="{{ $idx % 2 == 1 ? 'row-alt' : '' }}">
                        ...
                        <td class="text-center"><span class="badge-ali" style="{{ $es_extraordinario ? 'background-color: #f5f3ff; color: #6d28d9; border-color: #ddd6fe;' : '' }}">{{ $es_extraordinario ? 'EXT' : 'Ali ' . $aliGroup }}</span></td>
                ...
                <tr class="totals-bar">
                    <td colspan="3" class="text-right font-bold" style="padding: 2.5px 4px;">
                        {{ $es_extraordinario ? 'TOTAL PRESUPUESTO EXTRAORDINARIO:' : 'SUBTOTAL GASTOS COMUNES DEL MES:' }}
                    </td>
        ...
        <!-- 4. Resumen Financiero y Conversión Oficial BCV -->
        @php
            ...
            $tasaBcv = (float)($invoice->tasa_efectiva ?? ($condominio->tasa_cambio ?? 36.50));
